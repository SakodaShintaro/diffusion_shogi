...
def make_position_features(env, features):
    board = env.board
    # piece
    ...
    # 繰り返し数
    repetitions = env.repetition_hash[board.zobrist_hash()]
    features[42] = (repetitions - 1) / 3


def make_output_label(move, turn):
    if turn == WHITE:
...
def make_position_features_vec(envs, features_vec):
